...
class HoneyScraper:
    ...
    def _save_store_to_db(self, domain: str, store_id: str, partial_url: str, details: Dict):
        ...
        try:
            ...
            conn.rollback()
            logger.exception("Error saving store %s to database", store_id)
        finally:
            conn.close()
    ...
    def get_supported_domains(self) -> List[str]:
        ...
        try:
            ...
            logger.exception(f"Error fetching domains")
            return []
    ...
    def get_store_ids_by_domain(self, domain: str) -> List[Dict]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.exception(f"  ⚠️ Request error for {domain}. Retry {attempt + 1}/{max_retries}...")
                retry_delay *= 1.5
                if attempt < max_retries - 1:
                    time.sleep(retry_delay)
                    continue
            except Exception:
                logger.exception(f"Error fetching store IDs for {domain}")
                break
    ...
    def get_store_details(self, store_id: str, max_ugc: int = 3, success_count: int = 1) -> Optional[Dict]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                logger.exception(f"    ⚠️ Request error for store {store_id}. Retry {attempt + 1}/{max_retries}...")
                retry_delay *= 1.5
                if attempt < max_retries - 1:
                    time.sleep(retry_delay)
                    continue
            except Exception:
                logger.exception(f"Error fetching store details for {store_id}")
                break
    ...
    def scrape_all_stores(self, max_domains: Optional[int] = None, skip_existing: bool = True):
        ...
        for i, domain in enumerate(domains, 1):
            ...
            # Progress update
            if i % 100 == 0:
                logger.info(f"Progress: {processed} stores saved, {skipped} domains skipped, {errors} errors")

        elapsed = datetime.now() - start_time
        logger.info(
            "Scraping complete | domains=%d skipped=%d saved=%d errors=%d elapsed=%s db=%s",
            len(domains), skipped, processed, errors, elapsed, self.db_path
        )
    ...
    if len(sys.argv) > 1:
        ...
        if mode in ['auto', 'service', 'resume']:
            ...
            try:
                ...
                logger.exception(f"Invalid limit argument: {mode}")
                sys.exit(1)
